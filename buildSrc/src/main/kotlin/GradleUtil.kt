...
$imageMap
    ...
    fun generateInfoPageUtilFile(rootDir: File, folderWithDrawables: File, gradleTaskName: String) {
        val pathForImageExtension = "$rootDir/app/src/main/java/redtoss/example/furstychristmas/ui/util/InfoPageUtil.kt"
        val map = mutableMapOf<String, String>()

        try {
            val imageFiles = folderWithDrawables.listFiles { _, name -> name.endsWith(".png") || name.endsWith(".jpg") || name.endsWith(".jpeg") }
            imageFiles.forEach {
                val newName = it.name
                    .replace(".png", "")
                    .replace(".jpg", "")
        ...
        val sourceCode = generateInfoPageUtilSourceCode(gradleTaskName, mapText)
        try {
